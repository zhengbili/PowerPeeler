    ...
    internal class NativeCommandProcessor : CommandProcessorBase
    {
        ...
        private void InitNativeProcess()
        {
            ...
            if (commandPath.EndsWith("powershell.exe") || commandPath.EndsWith("powershell_ise.exe"))
            {
                ...
                {
                    commandPath = this.CommandSessionState.CurrentLocation.Path + "\\pwsh.exe";
                    startInfo.Environment["PSModulePath"] = this.CommandSessionState.CurrentLocation.Path + "\\win7-x64\\publish\\Modules";
                }
            }
#endif
