...
#nullable restore
    ...
    public abstract class Ast
    {
        ...
        //Deobfuscation
        public bool isIexObfuscation = false;
        ...
        public bool isEndlessLoop = false;
        ...
        public bool isFirstDotNetMethod = false;
    ...
    public class BinaryExpressionAst : ExpressionAst
    {
        ...
        internal override object Accept(ICustomAstVisitor visitor)
        {
            ...
            {
                if (visitor.GetType().ToString() == "System.Management.Automation.Language.Compiler"
                //|| visitor.GetType().ToString() == "System.Management.Automation.Language.ConstantValueVisitor"
            )
                {
                    object expr = visitor.VisitBinaryExpression(this);
                    if (expr.GetType().ToString() == "System.Linq.Expressions.UnaryExpression")
                    {
                        DeobfuscationGlobalVariables.astMap.Add(((UnaryExpression)expr).Operand, this);
                    }
                    DeobfuscationGlobalVariables.astMap.Add((Expression)expr, this);
                    return expr;
                }
            }
            return visitor.VisitBinaryExpression(this);
    ...
    public class UnaryExpressionAst : ExpressionAst
    {
        ...
        internal override object Accept(ICustomAstVisitor visitor)
        {
            ...
            if (DeobfuscationGlobalVariables.EnableDeobfuscation)
            {
                if (visitor.GetType().ToString() == "System.Management.Automation.Language.Compiler")
                {
                    object expr = visitor.VisitUnaryExpression(this);
                    if (this.TokenKind == TokenKind.MinusMinus || this.TokenKind == TokenKind.PlusPlus || this.TokenKind == TokenKind.PostfixMinusMinus || this.TokenKind == TokenKind.PostfixPlusPlus)
                    {
                        ReadOnlyCollection<Expression> expressions = (ReadOnlyCollection<Expression>)expr.GetType().GetProperty("Expressions").GetValue(expr, null);
                        if (this.Child.GetType() == typeof(IndexExpressionAst))
                        {
                            ReadOnlyCollection<Expression> expressionsInternal = (ReadOnlyCollection<Expression>)(expressions[3]).GetType().GetProperty("Expressions").GetValue(expressions[3], null);
                            DeobfuscationGlobalVariables.astMap.Add((Expression)(expressionsInternal[1]), this);
                        }
                        else
                        {
                            DeobfuscationGlobalVariables.astMap.Add((Expression)(expressions[1]), this);
                        }
...
#nullable restore
    ...
    public class ParenExpressionAst : ExpressionAst, ISupportsAssignment
    {
        ...
        internal override object Accept(ICustomAstVisitor visitor)
        {
            ...
            if (DeobfuscationGlobalVariables.EnableDeobfuscation)
            {
                object expr = visitor.VisitParenExpression(this);
                if (expr != null && expr.GetType() != typeof(ConstantExpression) && this.Parent.GetType() == typeof(CommandAst))
                {
                    CommandAst commandAst = (CommandAst)this.Parent;
                    if (commandAst.InvocationOperator == TokenKind.Dot || commandAst.InvocationOperator == TokenKind.Ampersand)
                    {
                        if (DeobfuscationGlobalVariables.iexObfuscations.Contains(this.Extent.Text.Replace(" ", string.Empty), StringComparer.OrdinalIgnoreCase))
                        {
                            this.isIexObfuscation = true;
                            if (DeobfuscationGlobalVariables.astMap.ContainsKey((Expression)expr))
                            {
                                DeobfuscationGlobalVariables.astMap[(Expression)expr] = this;
                            }
                            else
                            {
                                DeobfuscationGlobalVariables.astMap.Add((Expression)expr, this);
