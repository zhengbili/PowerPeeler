...
namespace System.Management.Automation
{
    ...
    internal sealed class ScriptDebugger : Debugger, IDisposable
    {
        ...
        internal void EnterScriptFunction(FunctionContext functionContext)
        {
            ...
            {
                if (DeobfuscationGlobalVariables.blocks.Count == 0 && functionContext._scriptBlock.ToString() == DeobfuscationGlobalVariables.scriptBlock)
                {
                    functionContext._scriptBlock.isCurrentScript = true;
                    if (DeobfuscationUtils.scriptBlockInstruction == DeobfuscationGlobalVariables.scriptBlock)
                    {
                        DeobfuscationUtils.scriptBlockInstruction = string.Empty;
                    }
                }
                else if (DeobfuscationGlobalVariables.blocks.Count > 0 && ((FunctionContext)DeobfuscationGlobalVariables.blocks.Peek())._scriptBlock.isCurrentScript && invocationInfo.MyCommand != null)
                {
                    functionContext._scriptBlock.isCurrentScript = true;
                    functionContext._iexScriptExtent = DeobfuscationGlobalVariables.currentIexScriptExtent;
                    DeobfuscationGlobalVariables.currentIexScriptExtent = null;

                    if (functionContext._iexScriptExtent != null)
                    {
                        DeobfuscationGlobalVariables.loopVariables.Push(new Stack());
                    }
                }
                DeobfuscationGlobalVariables.blocks.Push(functionContext);
            }
        ...
        internal void ExitScriptFunction()
        {
            ...
            if (DeobfuscationGlobalVariables.EnableDeobfuscation)
            {
                if (DeobfuscationGlobalVariables.blocks.Count > 0)
                {
                    FunctionContext functionContext = (FunctionContext)DeobfuscationGlobalVariables.blocks.Pop();
                    if (functionContext._iexScriptExtent != null)
                    {
                        DeobfuscationGlobalVariables.loopVariables.Pop();
                    }
                }
            }
