...
namespace System.Management.Automation
{
    ...
    internal abstract class CommandProcessorBase : IDisposable
    {
        ...
        //Deobfuscation
        internal int[][] iexOffsetArray { get; set; }

        internal int[][] functionOffsetArray { get; set; }

        internal Dictionary<string, object> currentCommandMap { get; set; }
        ...
        internal void DoExecute()
        {
            ...
            try
            {
                ...
                if (DeobfuscationGlobalVariables.EnableDeobfuscation)
                {
                    try
                    {
                        if (DeobfuscationGlobalVariables.blocks.Count > 0 && ((FunctionContext)DeobfuscationGlobalVariables.blocks.Peek())._scriptBlock.isCurrentScript && DeobfuscationGlobalVariables.currentInstruction != null)
                        {
                            if (DeobfuscationGlobalVariables.EnableRecord)
                            {
                                ...
                                RecordMap["sinkFunction"] = CommandInfo.Name;
                                RecordMap["commandType"] = CommandInfo.CommandType.ToString();
                                RecordMap["startOffset"] = Command.InvocationExtent.StartOffset;
                                RecordMap["endOffset"] = Command.InvocationExtent.EndOffset;
                                if (iexOffsetArray.Length > 0)
                                {
                                    RecordMap["iexOffset"] = iexOffsetArray;
                                }
                                if (functionOffsetArray.Length > 0)
                                {
                                    RecordMap["functionOffset"] = functionOffsetArray;
                                }
                                DeobfuscationUtils.RecordSinkFunctionObject(RecordMap);
                            }

                            Dictionary<string, object> commandMap = new Dictionary<string, object>();
                            Dictionary<string, object>[] argues = new Dictionary<string, object>[arguments.Count];
                            int index = 0;

                            foreach (CommandParameterInternal argue in arguments)
                            {
                                argues[index] = new Dictionary<string, object>();

                                if (argue.ParameterExtent.Text == string.Empty)
                                {
                                    argues[index]["type"] = "arguement";
                                    argues[index]["valueType"] = argue.ArgumentValue.GetType().ToString();
                                    argues[index]["value"] = DeobfuscationUtils.DeepCopy(argue.ArgumentValue);
                                    argues[index]["startOffset"] = argue.ArgumentExtent.StartOffset;
                                    argues[index]["endOffset"] = argue.ArgumentExtent.EndOffset;
                                }
                                else
                                {
                                    argues[index]["type"] = "parameter";
                                    argues[index]["valueType"] = argue.ParameterName.GetType().ToString();
                                    argues[index]["value"] = DeobfuscationUtils.DeepCopy(argue.ParameterName);
                                    argues[index]["startOffset"] = argue.ParameterExtent.StartOffset;
                                    argues[index]["endOffset"] = argue.ParameterExtent.EndOffset;
                                }

                                index++;
                            }

                            commandMap["astType"] = "CommandAst";
                            commandMap["commandType"] = CommandInfo.CommandType.ToString();
                            commandMap["commandName"] = CommandInfo.Name;
                            commandMap["startOffset"] = Command.InvocationExtent.StartOffset;
                            commandMap["endOffset"] = Command.InvocationExtent.EndOffset;

                            if (iexOffsetArray.Length > 0)
                            {
                                commandMap["iexOffset"] = iexOffsetArray;
                            }
                            if (functionOffsetArray.Length > 0)
                            {
                                commandMap["functionOffset"] = functionOffsetArray;
                            }

                            if (argues.Length > 0 && CommandInfo.Name != "ForEach-Object")
                            {
                                commandMap["argues"] = argues;

                                if (CommandInfo.Name == "Set-Variable" || CommandInfo.Name == "New-Variable")
                                {
                                    for (int k = 0; k < argues.Length - 1; k++)
                                    {
                                        Dictionary<string, object> dic = argues[k];
                                        if ((string)dic["type"] == "parameter" && (string)dic["value"] == "Value")
                                        {
                                            Dictionary<string, object> dicNext = argues[k + 1];
                                            DeobfuscationUtils.RecordLoopVariable(dicNext["value"], Command.InvocationExtent.StartOffset, Command.InvocationExtent.EndOffset);
                                        }
                                    }
                                }
                            }
                            ...
                            if (argues.Length == 0 || CommandInfo.Name == "ForEach-Object")
                            {
                                if (CommandRuntime.InputPipe.ObjectQueue != null && CommandRuntime.InputPipe.ObjectQueue.Count > 0)
                                {
                                    commandMap["pipeInputType"] = CommandRuntime.InputPipe.ObjectQueue.Peek().GetType().ToString();
                                    commandMap["pipeInput"] = DeobfuscationUtils.DeepCopy(CommandRuntime.InputPipe.ObjectQueue.Peek());
                                }else if (CommandRuntime.InputPipe.GetType().GetField("_enumeratorToProcess", BindingFlags.NonPublic | BindingFlags.Instance).GetValue(CommandRuntime.InputPipe) != null)
                                {
                            ...
                            {
                                DeobfuscationUtils.WriteLog(commandMap);
                            }
                        }
                    }
                    catch (Exception e)
                    ...
                    if (this.GetType().ToString() == "System.Management.Automation.NativeCommandProcessor")
                    {
                        name = (string)this.GetType().GetProperty("NativeCommandName", BindingFlags.NonPublic | BindingFlags.Instance).GetValue(this);
                        if (DeobfuscationGlobalVariables.nativeBlacklist.Any(suffix => name.EndsWith(suffix, true, ci)))
                        {
                            throw new Exception("Downloading is disabled.");
                        }
                    }
                    else if (this.GetType().ToString() == "System.Management.Automation.CommandProcessor")
                    {
                        name = CommandInfo.Name;
                        if (DeobfuscationGlobalVariables.cmdletBlacklist.Any(suffix => name.Equals(suffix, StringComparison.OrdinalIgnoreCase)))
                        {
                            throw new Exception("Downloading is disabled.");
                        }
                        if (name == "New-Object")
                        {
                            if (arguments.Count > 0 && arguments[0].ArgumentExtent.Text.EndsWith("TCPClient"))
                            {
                                throw new Exception("Downloading is disabled.");
                            }
                        }
