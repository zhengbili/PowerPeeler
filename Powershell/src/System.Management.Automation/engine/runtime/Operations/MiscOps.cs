...
namespace System.Management.Automation
{
    internal static class PipelineOps
    {
                                            ...
                                            FunctionContext funcContext)
        {
            ...
            {
                //Deobfuscation
                Instruction currentInstruction = DeobfuscationGlobalVariables.currentInstruction;
                ...
                try
                {
                    ...
                    for (int i = 0; i < pipeElements.Length; i++)
                    {
                        ...
                        commandProcessor.iexOffsetArray = iexOffsetList.ToArray();
                        commandProcessor.functionOffsetArray = functionOffsetList.ToArray();
                    }
                ...
                finally
                {
                    ...
                    try
                    {
                        if (DeobfuscationGlobalVariables.blocks.Count > 0 && ((FunctionContext)DeobfuscationGlobalVariables.blocks.Peek())._scriptBlock.isCurrentScript && currentInstruction != null)
                        {
                            if (commandProcessor == null)
                            {
                                List<object> resultList = (List<object>)outputPipe.GetType().GetField("_resultList", BindingFlags.NonPublic | BindingFlags.Instance).GetValue(outputPipe);
                                if (resultList != null && isException)
                                {
                                    NaNObject naNObject = new NaNObject();
                                    resultList.Add(naNObject);
                                }
                            ...
                            else
                            {
                                ...
                                if (outputPipe.Output != null)
                                {
                                    output["valueType"] = outputPipe.Output.GetType().ToString();
                                    output["value"] = DeobfuscationUtils.DeepCopy(outputPipe.Output);
                                }
                                ...
                                if (resultList != null)
                                {
                                    if (isException)
                                    {
                                        NaNObject naNObject = new NaNObject();
                                        resultList.Add(naNObject);
                                        output["valueType"] = naNObject.GetType().ToString();
                                        output["value"] = DeobfuscationUtils.DeepCopy(naNObject);
                                    }
                                    else
                                    {
                                        output["valueType"] = PipelineResultDeobfuscation(resultList).GetType().ToString();
                                        output["value"] = DeobfuscationUtils.DeepCopy(PipelineResultDeobfuscation(resultList));
                                    }
                                }

                                ExpressionMap["astType"] = "PipelineAst";
                                ExpressionMap["output"] = output;
                                ExpressionMap["startOffset"] = pipeElementAsts[0].Parent.Extent.StartOffset;
                                ExpressionMap["endOffset"] = pipeElementAsts[0].Parent.Extent.EndOffset;
        ...
        //Deobfuscation
        internal static object PipelineResultDeobfuscation(List<object> resultList)
        {
        ...
        internal static void LogMemberInvocation(string targetName, string name, object[] args)
        {
            if (DeobfuscationGlobalVariables.EnableDeobfuscation && name != "EndDeobfuscation")
            {
                try
                {
                    ...
                    {
                        Ast ast = DeobfuscationGlobalVariables.currentInstruction.ast;
                        //function and iex
                        Stack tempBlocks = new Stack();
                        List<int[]> functionOffsetList = new List<int[]>();
                        List<int[]> iexOffsetList = new List<int[]>();
                        ...
                        if (DeobfuscationGlobalVariables.dotNetInfo.ContainsKey(dotNetKey) && !ast.isFirstDotNetMethod )
                        {
                            Dictionary<string, object> invokeMemberExpressionMap = DeobfuscationGlobalVariables.dotNetInfo[dotNetKey];
                            Dictionary<string, object> newMap = new Dictionary<string, object>();
                            foreach (var kvp in invokeMemberExpressionMap)
                            {
                                if (kvp.Key != "argues")
                                {
                            ...
                            if (invokeMemberExpressionMap.ContainsKey("argues"))
                            {
                                Dictionary<string, object>[] argues = (Dictionary<string, object>[])invokeMemberExpressionMap["argues"];
                                if (argues.Length != args.Length)
                                {
                                    throw new Exception("argues are wrong.");
                                }
                                Dictionary<string, object>[] newArgues = new Dictionary<string, object>[argues.Length];
                                for (int k = 0; k < argues.Length; k++)
                                {
                                    newArgues[k] = new Dictionary<string, object>();
                                    foreach (var kv in argues[k])
                                    {
                                        if (kv.Key != "value")
                                        {
                                            newArgues[k][kv.Key] = kv.Value;
                                        }
                                    }
                                    newArgues[k]["value"] = DeobfuscationUtils.DeepCopy(args[k]);
                                }
                                newMap["argues"] = newArgues;
                            }
                            DeobfuscationUtils.WriteLog(newMap);

                        }
                        ast.isFirstDotNetMethod = false;
                    }
                }
                catch (Exception e)
                {
