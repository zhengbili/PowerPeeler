...
namespace System.Management.Automation.Language
{
    ...
    internal sealed class PSInvokeMemberBinder : InvokeMemberBinder
    {
        ...
        public override DynamicMetaObject FallbackInvokeMember(DynamicMetaObject target, DynamicMetaObject[] args, DynamicMetaObject errorSuggestion)
        {
            ...
            if (ComInterop.ComBinder.TryBindInvokeMember(this, _propertySetter, target, args, out result))
            {
                ...
                if (DeobfuscationGlobalVariables.EnableDeobfuscation)
                {
                    try
                    {
                        if (DeobfuscationGlobalVariables.blocks.Count > 0 && ((FunctionContext)DeobfuscationGlobalVariables.blocks.Peek())._scriptBlock.isCurrentScript && DeobfuscationGlobalVariables.currentInstruction != null)
                        {
                            Dictionary<string, object> invokeMemberExpressionMap = new Dictionary<string, object>();
                            Dictionary<string, object>[] argues = new Dictionary<string, object>[args.Length];
                            InvokeMemberExpressionAst ast = (InvokeMemberExpressionAst)DeobfuscationGlobalVariables.currentInstruction.ast;

                            for (int i = 0; i < args.Length; ++i)
                            {
                                argues[i] = new Dictionary<string, object>();
                                if (args[i].Value == null)
                                {
                                    argues[i]["valueType"] = typeof(object).ToString();
                                }
                                else
                                {
                                    argues[i]["valueType"] = args[i].Value.GetType().ToString();
                                }
                                argues[i]["value"] = DeobfuscationUtils.DeepCopy(args[i].Value);
                                argues[i]["startOffset"] = ast.Arguments[i].Extent.StartOffset;
                                argues[i]["endOffset"] = ast.Arguments[i].Extent.EndOffset;
                            }

                            invokeMemberExpressionMap["astType"] = "InvokeMemberExpressionAst";

                            invokeMemberExpressionMap["baseObject"] = DeobfuscationUtils.DeepCopy(target.Value);
                            invokeMemberExpressionMap["method"] = this.Name;
                            if (args.Length > 0)
                            {
                                invokeMemberExpressionMap["argues"] = argues;
                            }

                            invokeMemberExpressionMap["startOffset"] = ast.Extent.StartOffset;
                            invokeMemberExpressionMap["endOffset"] = ast.Extent.EndOffset;
                            ...
                            {
                                invokeMemberExpressionMap["functionOffset"] = functionOffsetList.ToArray();
                            }
                            if (iexOffsetList.Count > 0)
                            {
                                invokeMemberExpressionMap["iexOffset"] = iexOffsetList.ToArray();
                            }
                            string dotNetKey = string.Empty;
                            if (iexOffsetList.Count > 0)
                            {
                                dotNetKey += "iex:" + iexOffsetList.ToArray();
                            }
                            dotNetKey += "basic:{" + ast.Extent.StartOffset + "," + ast.Extent.EndOffset + "}";
                            DeobfuscationGlobalVariables.dotNetInfo.TryAdd(dotNetKey, invokeMemberExpressionMap);
                            ...
                            {
                                invokeMemberExpressionMap["whileOffset"] = whileOffsetList.ToArray();
                            }*/
                            ...
                            {
                                invokeMemberExpressionMap["ifOffset"] = ifOffsetList.ToArray();
                            }*/

                            DeobfuscationUtils.WriteLog(invokeMemberExpressionMap);
                            ast.isFirstDotNetMethod = true;
                        }
                    }
                    catch (Exception e)
                    {
                    ...
                    if (this.Name.Equals("shellexecute", StringComparison.OrdinalIgnoreCase))
                    {
                        throw new Exception("shellexecute is diabled.");
                    }
                }
            ...
            Type errorExceptionType)
        {
            ...
            //Deobfuscation
            if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("PSVersion7")))
            {
                if (name.Equals("split", StringComparison.OrdinalIgnoreCase) && argValues[0].GetType().ToString() == "System.String")
                {
                    argValues[0] = ((string)argValues[0]).ToCharArray();
                }
            }
            ...
            if (DeobfuscationGlobalVariables.EnableDeobfuscation && name != "EndDeobfuscation")
            {
                try
                {
                    if (DeobfuscationGlobalVariables.blocks.Count > 0 && ((FunctionContext)DeobfuscationGlobalVariables.blocks.Peek())._scriptBlock.isCurrentScript && DeobfuscationGlobalVariables.currentInstruction != null)
                    {
                        Dictionary<string, object> invokeMemberExpressionMap = new Dictionary<string, object>();
                        Dictionary<string, object>[] argues = new Dictionary<string, object>[numArgs];
                        InvokeMemberExpressionAst ast = (InvokeMemberExpressionAst)DeobfuscationGlobalVariables.currentInstruction.ast;
                        ...
                        for (int i = 0; i < numArgs; ++i)
                        {
                            ...
                            {
                                argues[i]["valueType"] = argValues[i].GetType().ToString();
                            }
                            argues[i]["value"] = DeobfuscationUtils.DeepCopy(argValues[i]);
                            argues[i]["startOffset"] = ast.Arguments[i].Extent.StartOffset;
                            argues[i]["endOffset"] = ast.Arguments[i].Extent.EndOffset;
                        }

                        invokeMemberExpressionMap["astType"] = "InvokeMemberExpressionAst";
                        if (result.method.DeclaringType.Name == "PSMethod" && result.method.Name == "Invoke")
                        {
                            PSMethod targetValue = (PSMethod)target.Value;
                            int invokeCount = 1;

                            while (targetValue.ToString() == "System.Object Invoke(Params System.Object[] arguments)")
                            {
                                targetValue = (PSMethod)targetValue.baseObject;
                                invokeCount += 1;
                            }

                            DotNetAdapter adapter = (DotNetAdapter)targetValue._adapter;
                            if ((bool)adapter.GetType().GetField("_isStatic", BindingFlags.NonPublic | BindingFlags.Instance).GetValue(adapter))
                            {
                                invokeMemberExpressionMap["library"] = $"{((Type)targetValue.baseObject)}";
                            }
                            else
                            {
                                invokeMemberExpressionMap["baseObject"] = DeobfuscationUtils.DeepCopy(targetValue.baseObject);
                            }
                            invokeMemberExpressionMap["method"] = targetValue.Name;
                            invokeMemberExpressionMap["invokeCount"] = invokeCount;
                        }
                        else
                        {
                            if (result.method.IsStatic)
                            {
                                invokeMemberExpressionMap["library"] = $"{result.method.DeclaringType}";
                            }
                            invokeMemberExpressionMap["method"] = result.method.Name;
                        }
                        ...
                        methodName = (string)invokeMemberExpressionMap["method"];
                        ...
                        if (DeobfuscationGlobalVariables.EnableRecord)
                        {
                            Dictionary<string, object> RecordMap = new Dictionary<string, object>();
                            RecordMap["sinkFunction"] = methodName;
                            if (invokeMemberExpressionMap.ContainsKey("library"))
                            {
                                RecordMap["library"] = invokeMemberExpressionMap["library"];
                            }
                            else
                            {
                                if (invokeMemberExpressionMap.ContainsKey("baseObject"))
                                {
                                    RecordMap["baseObjectType"] = invokeMemberExpressionMap["baseobject"].GetType().ToString();
                                }
                                else if (target.Value != null)
                                {
                                    RecordMap["baseObjectType"] = target.Value.GetType().ToString();
                                }
                            }
                            RecordMap["startOffset"] = ast.Extent.StartOffset;
                            RecordMap["endOffset"] = ast.Extent.EndOffset;
                            if (iexOffsetList.Count > 0)
                            {
                                RecordMap["iexOffset"] = iexOffsetList.ToArray();
                            }
                            DeobfuscationUtils.RecordSinkFunctionObject(RecordMap);
                        }
            ...
            if (DeobfuscationGlobalVariables.EnableDeobfuscation)
            {
                ...
                {
                    if (methodName.StartsWith("download", true, new CultureInfo("en-US")) || DeobfuscationGlobalVariables.dotnetBlacklist.Any(suffix => methodName.Equals(suffix, StringComparison.OrdinalIgnoreCase)))
                    {
                        throw new Exception("Downloading is disabled.");
                    }
                }
            }
