...
#pragma warning restore IDE0044 // Variable is assigned when DEBUG is defined.
        ...
        public void Emit(Instruction instruction)
        {
            ...
            {
                if (DeobfuscationGlobalVariables.EnableInstructionsRecord)
                {
                    if (DeobfuscationGlobalVariables.blocks.Count == 0 && DeobfuscationUtils.scriptBlockInstruction == DeobfuscationGlobalVariables.scriptBlock)
                    {
                        DeobfuscationUtils.instructions++;
                    }
                    else if (DeobfuscationGlobalVariables.blocks.Count > 0 && ((FunctionContext)DeobfuscationGlobalVariables.blocks.Peek())._scriptBlock.isCurrentScript)
                    {
                        DeobfuscationUtils.instructions++;
                    }
                }
                    //Add ast to current instruction
                if (DeobfuscationGlobalVariables.currentExpr != null)
                {
                    Expression exp = DeobfuscationGlobalVariables.currentExpr;
                    DeobfuscationGlobalVariables.astMap.TryGetValue(DeobfuscationGlobalVariables.currentExpr, out Ast ast);
                    instruction.ast = ast;
        ...
        public void EmitBranchFalse(BranchLabel elseLabel)
        {
            ...
            /*if (DeobfuscationGlobalVariables.EnableDeobfuscation)
            {
                BranchFalseInstruction instr = new BranchFalseInstruction();
                if (_instructions[_instructions.Count - 1].ast != null
                    && _instructions[_instructions.Count - 1].ast.GetType().ToString() == "System.Management.Automation.Language.WhileStatementAst")
                {
                    instr.ast = _instructions[_instructions.Count - 1].ast;
                }
                EmitBranch(instr, elseLabel);
            }
            else
            {*/
