...
begin {
    ...
    function Serialize ($Object,$Iteration,$Indent) {
        ...
        function QuoteChar ([char[]]$Item) {
            if($check -and $Item.Count -gt $global:Rule['MAX_LENGTH']){throw 'Two many chars!'}
            #$t = -join ($Item | ForEach-Object { if ($_ -cmatch $PrintableChar){([string]$_).Replace('`','``').Replace('$','`$').Replace('"','`"')}else{'`u{{{0:x2}}}' -f [int]$_} })
            $t=[DeobfuscationHelper.Stringify]::QuoteChar($Item);
            if ($t -match '[\r\n]'){ "@`"$NewLine${t}$NewLine`"@" } else { "`"${t}`"" }
        }
        function QuoteByte ([byte[]]$Item) {
            if($check -and $Item.Count -gt $global:Rule['MAX_LENGTH']){throw 'Two many chars!'}
            #$rate = ($Item | ForEach-Object { if ([char]$_ -cmatch $PrintableChar){1}else{0} } | Measure-Object -Sum).Sum / $Item.Length;
            $rate = [DeobfuscationHelper.Stringify]::PrintableRate($Item);
            if($check -and $rate -lt $global:Rule['CHECK_PRINTABLE_RATE']){throw 'bytes with too many non-ascii characters!'}
            "[char[]]$(QuoteChar([char[]]$Object))"
        }
        #function Here ([string]$Item) { if ($Item -match '[\r\n]') { "@`"$NewLine$($Item.Replace('`','``').Replace('$','`$').Replace('@','`@'))$NewLine`"@$NewLine" } else { Quote $Item } }
        function Here ([string]$Item) { QuoteChar([char[]]$Item) }
        function Stringify ($Object,$Cast = $Type,$Convert) {
            ...
            function Iterate ($Object,[switch]$Strong = $Strong,[switch]$ListItem,[switch]$Level) {
                if ($Iteration -lt $Depth) { Serialize $Object -Iteration ($Iteration + 1) -Indent ($Indent + 1 - [int][bool]$Level) }
                else { if($check){throw 'Too large depth!'}else{"'...'"} }
            }
            if ($Object -is [string]) { Prefix $Object } else {
                ...
                if ($Methods['GetEnumerator'] -is [PSMethod]) {
                    if ($Methods['get_Keys'] -is [PSMethod] -and $Methods['get_Values'] -is [PSMethod]) {
                        ...
                        if($check -and @($Object).Count -gt $global:Rule['MAX_LENGTH']){throw 'Two many objects!'}
        ...
        if ($Null -eq $Object) { "`$Null" } else {
            ...
            elseif ($Object -is [double] -or $Object -is [single]) {
                if("$Object" -eq "NaN"){Stringify "::NaN" $Type}
                elseif("$Object" -eq "Infinity"){Stringify "::PositiveInfinity" $Type}
                elseif("$Object" -eq "-Infinity"){Stringify "::NegativeInfinity" $Type}
