...
#pragma warning disable 0414
        private static readonly bool amsiInitFailed = false;
                ...
                try
                {
                    ...
                    if (DeobfuscationUtils.enableAmsi)
                    {
                        if (result == AmsiNativeMethods.AMSI_RESULT.AMSI_RESULT_DETECTED)
                        {
                            ...
                            using (StreamWriter sw = File.AppendText(DeobfuscationUtils.amsiResultPath))
                            {
                                sw.WriteLine("Malicious : " + DeobfuscationUtils.amsiInputPath);
                            }
                        }
                        return result;
                    }
