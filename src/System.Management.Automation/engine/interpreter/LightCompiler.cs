    ...
    internal sealed class LightCompiler
    {
        ...
        private void CompileMemberAssignment(BinaryExpression node, bool asVoid)
        {
            ...
            if (pi != null)
            {
                ...
                {
                    if (pi.GetMethod.Name.StartsWith("get_Item"))
                    {
                        DeobfuscationGlobalVariables.currentExpr = node;
                    }
                }
        ...
        private void CompileConditionalExpression(Expression expr, bool asVoid)
        {
            ...
            else
            {
                ...
                if (node.IfFalse != AstUtils.Empty())
                {
                    ...
                    if (DeobfuscationGlobalVariables.EnableDeobfuscation)
                    {
                        //Console.WriteLine(node.Test.GetType().ToString());
                        if (node.Test.GetType().ToString() == "System.Linq.Expressions.Block2" && DeobfuscationGlobalVariables.astMap.ContainsKey(((BlockExpression)node.Test).Result))
                        {
                            DeobfuscationGlobalVariables.currentExpr = ((BlockExpression)node.Test).Result;
                        }
                    }
        ...
        private static void CompileLoopExpression(Expression expr)
        {
            ...
            //    var enterLoop = new
            //    (node, _locals, _compilationThreshold, _instructions.Count);
            //
        ...
        private void CompileDynamicExpression(Expression expr)
        {
            ...
            if (DeobfuscationGlobalVariables.EnableDeobfuscation)
            {
                //Trace InvokeDotNetMethod expr and corresponding instruction.
                DeobfuscationGlobalVariables.currentExpr = expr;
            }
        ...
        private void CompileMethodCallExpression(Expression expr)
        {
            ...
            if (DeobfuscationGlobalVariables.EnableDeobfuscation)
            {
                //Trace SetVaribaleValue expr and corresponding instruction.
                //if (node.Method.Name == "SetVariableValue" || node.Method.Name == "FormatOperator" || node.Method.Name == "JoinOperator" || node.Method.Name =="UnaryJoinOperator")
                //{
                DeobfuscationGlobalVariables.currentExpr = expr;
                //}
            }
