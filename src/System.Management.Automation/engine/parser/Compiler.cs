...
namespace System.Management.Automation.Language
{
    ...
    internal class FunctionContext
    {
        ...
        //Deobfuscation
        internal IScriptExtent _iexScriptExtent;
    ...
    internal class Compiler : ICustomAstVisitor2
    {
        ...
        public object VisitIfStatement(IfStatementAst ifStmtAst)
        {
            ...
            for (int i = 0; i < clauseCount; ++i)
            {
                ...
                {
                    Expression expression = CaptureStatementResults(ifClause.Item1, CaptureAstContext.Condition).Convert(typeof(bool));
                    ...
                    /*if (DeobfuscationGlobalVariables.astMap.ContainsKey(expression))
                    {
                        DeobfuscationGlobalVariables.astMap.TryGetValue(expression, out Ast resultAst);
                        Console.WriteLine(resultAst.GetType());
                    }
                    DeobfuscationGlobalVariables.astMap.Add(expression, ifClause.Item1);*/
                }
                else
                {
                    var cond = UpdatePositionForInitializerOrCondition(
            ...
            MergeRedirectExprs generateRedirectExprs = null)
        {
            ...
            if (DeobfuscationGlobalVariables.EnableDeobfuscation)
            {
                //Add AssignmentStatementAst and corresponding expr.
                if (assignmentStatementAst.Left.GetType() == typeof(IndexExpressionAst))
                {
                    ReadOnlyCollection<Expression> expressions = (ReadOnlyCollection<Expression>)exprs.Last().GetType().GetProperty("Expressions").GetValue(exprs.Last(), null);
                    if (assignmentStatementAst.Operator == TokenKind.RemainderEquals || assignmentStatementAst.Operator == TokenKind.MinusEquals || assignmentStatementAst.Operator == TokenKind.PlusEquals || assignmentStatementAst.Operator == TokenKind.MultiplyEquals || assignmentStatementAst.Operator == TokenKind.DivideEquals)
                    {
                        ReadOnlyCollection<Expression> expressionsInternel = (ReadOnlyCollection<Expression>)expressions.Last().GetType().GetProperty("Expressions").GetValue(expressions.Last(), null);
                        DeobfuscationGlobalVariables.astMap.Add(expressionsInternel[1], assignmentStatementAst);
                    }
                    else
                    {
                        DeobfuscationGlobalVariables.astMap.Add(expressions[1], assignmentStatementAst);
                    }
                }
                else if (assignmentStatementAst.Operator == TokenKind.RemainderEquals || assignmentStatementAst.Operator == TokenKind.MinusEquals || assignmentStatementAst.Operator == TokenKind.PlusEquals || assignmentStatementAst.Operator == TokenKind.MultiplyEquals || assignmentStatementAst.Operator == TokenKind.DivideEquals)
                {
                    ReadOnlyCollection<Expression> expressions = (ReadOnlyCollection<Expression>)exprs.Last().GetType().GetProperty("Expressions").GetValue(exprs.Last(), null);
                    DeobfuscationGlobalVariables.astMap.Add(expressions[0], assignmentStatementAst);
                }
                else
                {
                    DeobfuscationGlobalVariables.astMap.Add(exprs.Last(), assignmentStatementAst);
                }
            }
        ...
        private Expression GenerateWhileLoop(
                                             string loopLabel,
                                             ...
                                             PipelineBaseAst continueAst = null)
        {
            ...
            if (generateCondition != null)
            {
                ...
                if (DeobfuscationGlobalVariables.EnableDeobfuscation)
                {
                    ConditionalExpression expression = Expression.IfThen(generateCondition().Convert(typeof(bool)), loopBody);
                    if (DeobfuscationGlobalVariables.whileAst.Count > 0)
                    {
                        DeobfuscationGlobalVariables.astMap.Add(expression.Test, (Ast)DeobfuscationGlobalVariables.whileAst.Peek());
                        DeobfuscationGlobalVariables.whileAst.Pop();
                    }
        ...
        {
            //Deobfuscation
            object result = GenerateDoLoop(doWhileStatementAst);

            //Deobfuscation
            if (DeobfuscationGlobalVariables.EnableDeobfuscation)
            {
                DeobfuscationGlobalVariables.whileAst.Push(doWhileStatementAst);
            }

            PipelineBaseAst condition = doWhileStatementAst.Condition;

            if (DeobfuscationGlobalVariables.EnableDeobfuscation)
            {
                IEnumerable<Expression> exprs = (IEnumerable<Expression>)(result.GetType().GetField("_exprs", BindingFlags.NonPublic | BindingFlags.Instance).GetValue(result));
                IsConstantValueVisitor.IsConstant(condition, out object constantValue);

                try
                {
                    if (constantValue != null)
                    {
                        string constantType = constantValue.GetType().ToString();
                        if ((constantType == "System.Boolean" && (bool)constantValue) || (constantType == "System.RuntimeType" && constantValue.ToString() == "System.Boolean") || (constantType == "System.Int32" && (int)constantValue != 0))
                        {
                            doWhileStatementAst.isEndlessLoop = true;
                            foreach (StatementAst ast in doWhileStatementAst.Body.Statements)
                            {
                                if (ast.GetType() == typeof(BreakStatementAst))
                                {
                                    doWhileStatementAst.isEndlessLoop = false;
                                    break;
                                }

                                if (ast.GetType() == typeof(IfStatementAst))
                                {
                                    foreach (Tuple<PipelineBaseAst, StatementBlockAst> astInternal in ((IfStatementAst)ast).Clauses)
                                    {
                                        foreach (StatementAst astStatement in astInternal.Item2.Statements)
                                        {
                                            if (astStatement.GetType() == typeof(BreakStatementAst))
                                            {
                                                doWhileStatementAst.isEndlessLoop = false;
                                                break;
                                            }
                                    ...
                                    foreach (StatementAst astStatement in ((IfStatementAst)ast).ElseClause.Statements)
                                    {
                                        if (astStatement.GetType() == typeof(BreakStatementAst))
                                        {
                                            doWhileStatementAst.isEndlessLoop = false;
                ...
                DeobfuscationGlobalVariables.astMap.Add(Enumerable.ElementAt<Expression>(exprs, 1), doWhileStatementAst);
        ...
        {
            //Deobfuscation
            object result = GenerateDoLoop(doUntilStatementAst);

            //Deobfuscation
            if (DeobfuscationGlobalVariables.EnableDeobfuscation)
            {
                DeobfuscationGlobalVariables.whileAst.Push(doUntilStatementAst);
            }

            PipelineBaseAst condition = doUntilStatementAst.Condition;
            ...
            if (DeobfuscationGlobalVariables.EnableDeobfuscation)
            {
                ...
                try
                {
                    ...
                    {
                        string constantType = constantValue.GetType().ToString();
                        if ((constantType == "System.Boolean" && !(bool)constantValue) || (constantType == "System.Int32" && (int)constantValue == 0))
                        {
                            doUntilStatementAst.isEndlessLoop = true;
                            foreach (StatementAst ast in doUntilStatementAst.Body.Statements)
                            {
                                if (ast.GetType() == typeof(BreakStatementAst))
                                {
                                    doUntilStatementAst.isEndlessLoop = false;
                                    break;
                                }
                ...
                DeobfuscationGlobalVariables.astMap.Add(Enumerable.ElementAt<Expression>(exprs, 1), doUntilStatementAst);
        ...
        public object VisitForStatement(ForStatementAst forStatementAst)
        {
            ...
            if (DeobfuscationGlobalVariables.EnableDeobfuscation)
            {
                DeobfuscationGlobalVariables.whileAst.Push(forStatementAst);
            }
            ...
            if (DeobfuscationGlobalVariables.EnableDeobfuscation)
            {
                IEnumerable<Expression> exprs = (IEnumerable<Expression>)(loop.GetType().GetField("_exprs", BindingFlags.NonPublic | BindingFlags.Instance).GetValue(loop));
                try
                {
                    if (condition == null)
                    {
                        forStatementAst.isEndlessLoop = true;
                        foreach (StatementAst ast in forStatementAst.Body.Statements)
                        {
                            if (ast.GetType() == typeof(BreakStatementAst))
                            {
                                forStatementAst.isEndlessLoop = false;
                                break;
                            }
                ...
                DeobfuscationGlobalVariables.astMap.Add(Enumerable.ElementAt<Expression>(exprs, 1), forStatementAst);
            }
        ...
        public object VisitWhileStatement(WhileStatementAst whileStatementAst)
        {
            ...
            if (DeobfuscationGlobalVariables.EnableDeobfuscation)
            {
                DeobfuscationGlobalVariables.whileAst.Push(whileStatementAst);
            }
            ...
            if (DeobfuscationGlobalVariables.EnableDeobfuscation)
            {
                ...
                try
                {
                    if (constantValue != null)
                    {
                        ...
                        if ((constantType == "System.Boolean" && (bool)constantValue) || (constantType == "System.RuntimeType" && constantValue.ToString() == "System.Boolean") || (constantType == "System.Int32" && (int)constantValue != 0))
                        {
                            whileStatementAst.isEndlessLoop = true;
                            foreach (StatementAst ast in whileStatementAst.Body.Statements)
                            {
                                if (ast.GetType() == typeof(BreakStatementAst))
                                {
                                    whileStatementAst.isEndlessLoop = false;
                                    break;
                                }
                ...
                DeobfuscationGlobalVariables.astMap.Add(Enumerable.ElementAt<Expression>(exprs, 1), whileStatementAst);
            }
            return loop;
        }
        ...
        public object VisitBinaryExpression(BinaryExpressionAst binaryExpressionAst)
        {
            ...
            if (!CompilingConstantExpression && IsConstantValueVisitor.IsConstant(binaryExpressionAst, out constantValue))
            {
                ...
                if (DeobfuscationGlobalVariables.EnableDeobfuscation)
                {
                    try
                    {
                        if (binaryExpressionAst.Parent.GetType() != typeof(BinaryExpressionAst))
                        {
                            Dictionary<string, object> ExpressionMap = new Dictionary<string, object>();
                            Dictionary<string, object> output = new Dictionary<string, object>();
                            output["valueType"] = constantValue.GetType().ToString();
                            output["value"] = DeobfuscationUtils.DeepCopy(constantValue);
                            ExpressionMap["astType"] = "System.Management.Automation.Language.BinaryExpressionAst";
                            ExpressionMap["output"] = output;
                            ExpressionMap["startOffset"] = binaryExpressionAst.Extent.StartOffset;
                            ExpressionMap["endOffset"] = binaryExpressionAst.Extent.EndOffset;
                            ExpressionMap["isConstant"] = true;
        ...
        public object VisitUnaryExpression(UnaryExpressionAst unaryExpressionAst)
        {
            ...
            if (!CompilingConstantExpression && IsConstantValueVisitor.IsConstant(unaryExpressionAst, out constantValue))
            {
                ...
                if (DeobfuscationGlobalVariables.EnableDeobfuscation)
                {
                    try
                    {
                        if (unaryExpressionAst.Parent.GetType() != typeof(UnaryExpressionAst))
                        {
                            Dictionary<string, object> ExpressionMap = new Dictionary<string, object>();
                            Dictionary<string, object> output = new Dictionary<string, object>();
                            output["valueType"] = constantValue.GetType().ToString();
                            output["value"] = DeobfuscationUtils.DeepCopy(constantValue);
                            ExpressionMap["astType"] = "System.Management.Automation.Language.UnaryExpressionAst";
                            ExpressionMap["output"] = output;
                            ExpressionMap["startOffset"] = unaryExpressionAst.Extent.StartOffset;
                            ExpressionMap["endOffset"] = unaryExpressionAst.Extent.EndOffset;
                            ExpressionMap["isConstant"] = true;
                            DeobfuscationUtils.WriteLog(ExpressionMap);
                        }
                    }
        ...
        public object VisitInvokeMemberExpression(InvokeMemberExpressionAst invokeMemberExpressionAst)
        {
            ...
            if (memberNameAst != null)
            {
                ...
                if (DeobfuscationGlobalVariables.EnableDeobfuscation)
                {
                    //Add InvokeMemberExpressionAst and corresponding expr.
                    expr = InvokeMember(
                        memberNameAst.Value,
                        constraints,
                        target,
                    ...
                    DeobfuscationGlobalVariables.astMap.Add(expr, invokeMemberExpressionAst);
                    return expr;
                }
                else
                {
            ...
            {
                //Add InvokeMemberExpressionAst and corresponding expr.
                expr = InvokeDynamicMember(memberNameExpr, constraints, target, args, invokeMemberExpressionAst.Static, propertySet: false, invokeMemberExpressionAst.NullConditional);
                DeobfuscationGlobalVariables.astMap.Add(expr, invokeMemberExpressionAst);
                return expr;
            }
            else
        ...
        public object VisitExpandableStringExpression(ExpandableStringExpressionAst expandableStringExpressionAst)
        {
            ...
            //Deobfuscation
            Expression expr = Expression.Call(CachedReflectionInfo.StringOps_FormatOperator, left, right);
            DeobfuscationGlobalVariables.astMap.Add(expr, expandableStringExpressionAst);
            return expr;
        }
        ...
        public void AddInstructions(LightCompiler compiler)
        {
            ...
            foreach (var expr in _exprs)
            {
                ...
                if (enterLoopExpression != null)
                {
                    ...
                    if (DeobfuscationGlobalVariables.EnableDeobfuscation)
                    {
                        DeobfuscationGlobalVariables.astMap.TryGetValue(expr, out Ast ast);
                        enterLoop.ast = ast;
                    }
                }
            }
