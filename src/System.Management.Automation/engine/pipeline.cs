        ...
        private void Start(bool incomingStream)
        {
            ...
            for (int i = 0; i < _commands.Count; i++)
            {
                ...
                if (DeobfuscationGlobalVariables.EnableDeobfuscation)
                {
                    ...
                    if (commandProcessor.GetType().ToString() == "System.Management.Automation.NativeCommandProcessor")
                    {
                        name = (string)commandProcessor.GetType().GetProperty("NativeCommandName", BindingFlags.NonPublic | BindingFlags.Instance).GetValue(commandProcessor);
                    ...
                    {
                        name = commandProcessor.CommandInfo.Name;
                        ...
                        if (name == "New-Object")
                        {
                            if (commandProcessor.arguments.Count > 0 && commandProcessor.arguments[0].ArgumentExtent.Text.EndsWith("TCPClient"))
                            {
                                isBlack = true;
                            }
                        }
