        ...
        internal sealed override bool Read()
        {
            ...
            while (!mandatoryParametersSpecified)
            {
                ...
                if (DeobfuscationGlobalVariables.EnableDeobfuscation)
                {
                    try
                    {
                        if (this.currentCommandMap != null && inputObject != AutomationNull.Value)
                        {
                            this.currentCommandMap["pipeInputType"] = inputObject.GetType().ToString();
                            this.currentCommandMap["pipeInput"] = DeobfuscationUtils.DeepCopy(inputObject);
                            DeobfuscationUtils.WriteLog(this.currentCommandMap);
                        }
                    }
                    catch (Exception e)
                    {
